...
fyf=84;     %--- First Year of Forecasting
...
%[Exp_Var]=deseasonal2(Exp_Var);
[Target, Exp_Var, Dummy]=transformation(Target_Level_X12, Exp_Var, Dummy, T);
%[Target, Exp_Var, Dummy]=transformation(Target, Exp_Var, Dummy, T);
...
%     [~,~, ~, ~, f_F2]=factoran(F2(1:sd-4-1+i,:), 2);
    ...
    for j=1:CC
    model(i,:,1+j)=ARDL(Target_,Exp_Var_(:,j),dum,OneStep);
    end
    %-------------------------------VAR------------------------------------
   ...
   % for k=1:size(v,1);
      %  Yraw=[dataset(:,1), dataset(:,v(k,:))];
        %-----------------------------------VAR----------------------------
       % model(i,:,(1+CC)+k)=BVAR(Yraw,dum,1,OneStep); % 20 model for var
        %----------------------------------TVP-VAR-------------------------
