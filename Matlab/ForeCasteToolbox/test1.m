...
Target=xlsread('Data4_level.xlsx', 'Target');
Exp_Var=xlsread('Data4_level.xlsx', 'Exp_Var');
% F1=xlsread('Data_level.xlsx', 'F1');
% F2=xlsread('Data_level.xlsx', 'F2');
Dummy=xlsread('Data4_level.xlsx', 'Dummy');
%---------------------Preaparing Data--------------------------------------
...
%     [~,~, ~, ~, f_F2]=factoran(F2(1:sd-horizon-1+i,:), 2);
    ...
    for j=1:CC
    model(i,:,1+j)=ARDL2(Target_,Exp_Var_(:,j),dum,OneStep, horizon);
    end
  %-------------------------------VAR------------------------------------
...
%     end
    ...
    ['Time remaining to compelete is:' num2str(round(tim*(nd+horizon-i))) 'Second(s)']
end

number_models=size(model,3);
%**************************************************************************
%**************************************************************************
psedu_outofsample=zeros(nd,horizon,number_models);
for i=1:horizon
psedu_outofsample(:,i,:)=model(9-i:end-i,i,:);
end

outofsample=zeros(horizon,number_models);
for i=1:horizon
...
RMSFE1=zeros(horizon,number_models);
for i=1:horizon
