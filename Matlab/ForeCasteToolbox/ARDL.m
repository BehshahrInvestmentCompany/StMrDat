...
for h=1:4;
    ...
    laginf=lagmatrix(inflation,h:h+4);
    lagother=lagmatrix(other, h:h+4);
    b=cell(5,5);
    for i=1:5;
        for j=1:5;
            m=max(h+i,h+j);
            ...
            switch d
                ...
                case 1
                    xx=[ones(length(y),1), laginf(m:end, 1:i), lagother(m:end, 1:j), dum(m:end-4,cn)];
            end
            b{i,j}=(xx'*xx)\xx'*y;
            yhat=xx*b{i,j};
            e=yhat-y;
    ...
    laginff=lagmatrix(inflation,[0 1 2 3 4]);
    lagotherf=lagmatrix(other, [0 1 2 3 4]);
    ...
    switch IC
        case 1
            if d==0
                ...
                yforecast(1,h)=[1, laginff(end,1:ra),lagotherf(end,1:ca), dum(end+h-4,cn)]*b{ra,ca};
            end
        case 2
            if d==0
                yforecast(1,h)=[1, laginff(end,1:rs),lagotherf(end,1:cs)]*b{rs,cs};
            else
                yforecast(1,h)=[1, laginff(end,1:rs),lagotherf(end,1:cs), dum(end+h-4,cn)]*b{rs,cs};
            end
        case 3
            if d==0
                yforecast(1,h)=[1, laginff(end,1:rh),lagotherf(end,1:ch)]*b{rh,ch};
            end
            yforecast(1,h)=[1, laginff(end,1:rh),lagotherf(end,1:ch), dum(end+h-4,cn)]*b{rh,ch};
    end
end
